#!/bin/bash
#
# Vendored + patched copy of gmag11/metatrader5_vnc's /Metatrader/start.sh.
# Bind-mounted over the image's copy (see docker-compose.yml) with ONE change:
# the mt5linux install is pinned to ==0.1.9. The stock script installs it
# unpinned, which now resolves to 1.0.3 whose CLI dropped the `-w` wine-python
# switch the [7/7] server-start line uses -> "Unknown switch -w" and no bridge.
# Re-diff against the upstream image if you bump the mt5 image tag.
#
...
# AutoTrading (Algo Trading) must be enabled ONCE in the terminal itself
# (VNC → toolbar "Algo Trading", or Tools > Options > Expert Advisors > "Allow
# Algo Trading"). It then persists in the mt5_config volume, same as the login.
# A startup-config ini with [Experts] Enabled=1 does NOT flip it in this Wine
# build (verified: terminal_info().trade_allowed stayed false, order_send → 10027)
# and risks resetting the saved state each boot, so we launch the terminal plainly
# and rely on the persisted setting.
...
# MetaTrader5 5.0.36's _core C-extension is built for the numpy 1.x ABI; an
# unpinned MetaTrader5 install pulls numpy 2.x, which breaks the import with
# "numpy.core.multiarray failed to import". Force the Wine numpy below 2 (pip
# picks the newest 1.x that still ships a cp39-win32 wheel).
if ! is_wine_python_package_installed "numpy<2"; then
...
if ! is_python_package_installed "mt5linux==0.1.9"; then
    # Pin 0.1.9: newer mt5linux (1.0.3) dropped the `-w` launcher switch this
    # image's [7/7] server-start line relies on, so unpinned installs break with
    # "Unknown switch -w". 0.1.9 keeps the wine-python rpyc launcher.
    pip install --break-system-packages --no-cache-dir --no-deps "mt5linux==0.1.9" && \
